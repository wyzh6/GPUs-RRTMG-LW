...
!------inatm in
real(kind = r8), device, allocatable ::  play_d(:,:)
real(kind = r8), device, allocatable :: plev_d(:,:)
real(kind = r8), device, allocatable ::  tlay_d(:,:)
real(kind = r8), device, allocatable ::  tlev_d(:,:)
real(kind = r8), device, allocatable ::  tsfc_d(:)
real(kind = r8), device, allocatable ::  h2ovmr_d(:,:)
real(kind = r8), device, allocatable ::  o3vmr_d(:,:)
real(kind = r8), device, allocatable ::  co2vmr_d(:,:)
real(kind = r8), device, allocatable ::  ch4vmr_d(:,:)
real(kind = r8), device, allocatable ::  o2vmr_d(:,:)
real(kind = r8), device, allocatable ::  n2ovmr_d(:,:)
real(kind = r8), device, allocatable ::  cfc11vmr_d(:,:)
real(kind = r8), device, allocatable ::  cfc12vmr_d(:,:)
real(kind = r8), device, allocatable ::  cfc22vmr_d(:,:)
real(kind = r8), device, allocatable ::  ccl4vmr_d(:,:)
real(kind = r8), device, allocatable ::  emis_d(:,:)
real(kind = r8), device, allocatable ::  cldfmcl_d(:, :, :)
real(kind = r8), device, allocatable ::  ciwpmcl_d(:, :, :)
real(kind = r8), device, allocatable ::  clwpmcl_d(:, :, :)
real(kind = r8), device, allocatable ::  reicmcl_d(:,:)
real(kind = r8), device, allocatable ::  relqmcl_d(:,:)
real(kind = r8), device, allocatable ::  taucmcl_d(:, :, :)
real(kind = r8), device, allocatable ::  tauaer_d(:,:,:)


!-------inatm use
integer, device, allocatable ::  ixindx_d(:)
!integer, device, allocatable ::  ngb_d(:)

!-------inatm out
real(kind = r8), device, allocatable ::  cldfmc(:,:,:)
real(kind = r8), device, allocatable ::  ciwpmc(:,:,:)
real(kind = r8), device, allocatable ::  clwpmc(:,:,:)
real(kind = r8), device, allocatable ::  relqmc(:,:)
real(kind = r8), device, allocatable ::  reicmc(:,:)
real(kind = r8), device, allocatable ::  dgesmc(:,:)
real(kind = r8), device, allocatable ::  taucmc(:,:,:)
real(kind = r8), device, allocatable ::  taua(:,:,:)
real(kind = r8), device, allocatable ::  pavel(:,:)
real(kind = r8), device, allocatable ::  tavel(:,:)
real(kind = r8), device, allocatable ::  pz(:,:)
real(kind = r8), device, allocatable ::  tz(:,:)
real(kind = r8), device, allocatable ::  coldry(:,:)
real(kind = r8), device, allocatable ::  wbrodl(:,:)
real(kind = r8), device, allocatable ::  wkl(:,:,:)
real(kind = r8), device, allocatable ::  wx(:,:,:)
real(kind = r8), device, allocatable ::  semiss(:,:)
real(kind = r8), device, allocatable ::  tbound(:)            
real(kind = r8), device, allocatable ::  pwvcm(:)   

!*******************************************************8
!----------------------cldprmc use
real(kind = r8), device, allocatable ::  absliq1_d(:,:)
real(kind = r8), device, allocatable ::  absice0_d(:)
real(kind = r8), device, allocatable ::  absice1_d(:,:)
real(kind = r8), device, allocatable ::  absice2_d(:,:)
real(kind = r8), device, allocatable ::  absice3_d(:,:)
integer, device, allocatable ::  ngb_d(:)

!-----------------------cldprmc in
 !real(kind = r8), device, allocatable ::  cldfmc(pcols,:,:)
!real(kind = r8), device, allocatable ::  ciwpmc(pcols,:,:)
!real(kind = r8), device, allocatable ::  clwpmc(pcols,:,:)
!real(kind = r8), device, allocatable ::  relqmc(pcols,:)
!real(kind = r8), device, allocatable ::  reicmc(pcols,:)
!real(kind = r8), device, allocatable ::  dgesmc(pcols,:)
!real(kind = r8), device, allocatable ::  taucmc(pcols,:,:)

!-----------------------cldprmc out
 !real(kind = r8), device, allocatable ::  taucmc(pcols,:,:)
integer, device, allocatable :: ncbands(:)
!**************************************************************
!--------------------setcoef use
 real(kind = r8), device, allocatable ::  totplnk_d(:,:)
 real(kind = r8), device, allocatable ::  totplk16_d(:)
 real(kind = r8), device, allocatable ::  preflog_d(:)
 real(kind = r8), device, allocatable ::  tref_d(:)
 real(kind = r8), device, allocatable ::  chi_mls_d(:,:)
!--------------------setcoef in
 !real(kind = r8), device, allocatable ::  pavel(:,:)
 !real(kind = r8), device, allocatable ::  tavel(:,:)
 !real(kind = r8), device, allocatable ::  tavel(:,:)          
 ...
 !---------------------setcoef out
integer, device, allocatable ::  laytrop(:)
integer, device, allocatable ::  jp(:,:)
integer, device, allocatable ::  jt(:,:)
integer, device, allocatable ::  jt1(:,:)
real(kind = r8), device, allocatable ::  planklay(:,:,:)
real(kind = r8), device, allocatable ::  planklev(:,:,:)
real(kind = r8), device, allocatable ::  plankbnd(:,:)
real(kind = r8), device, allocatable ::  colh2o(:,:)
real(kind = r8), device, allocatable ::  colco2(:,:)
real(kind = r8), device, allocatable ::  colo3(:,:)
real(kind = r8), device, allocatable ::  coln2o(:,:)
real(kind = r8), device, allocatable ::  colco(:,:)
real(kind = r8), device, allocatable ::  colch4(:,:)
real(kind = r8), device, allocatable ::  colo2(:,:)
real(kind = r8), device, allocatable ::  colbrd(:,:)
integer, device, allocatable ::  indself(:,:)
integer, device, allocatable ::  indfor(:,:)
real(kind = r8), device, allocatable ::  selffac(:,:)
real(kind = r8), device, allocatable ::  selffrac(:,:)
real(kind = r8), device, allocatable ::  forfac(:,:)
real(kind = r8), device, allocatable ::  forfrac(:,:)
integer, device, allocatable ::  indminor(:,:)
real(kind = r8), device, allocatable ::  minorfrac(:,:)
real(kind = r8), device, allocatable ::  scaleminor(:,:)
real(kind = r8), device, allocatable ::  scaleminorn2(:,:)
real(kind = r8), device, allocatable ::  fac00(:,:)
real(kind = r8), device, allocatable ::  fac01(:,:)
real(kind = r8), device, allocatable ::  fac10(:,:)
real(kind = r8), device, allocatable ::  fac11(:,:)

real(kind = r8), device, allocatable ::  rat_h2oco2(:,:)
real(kind = r8), device, allocatable ::  rat_h2oco2_1(:,:)
real(kind = r8), device, allocatable ::  rat_h2oo3(:,:)
real(kind = r8), device, allocatable ::  rat_h2oo3_1(:,:)
real(kind = r8), device, allocatable ::  rat_h2on2o(:,:)
real(kind = r8), device, allocatable ::  rat_h2on2o_1(:,:)
real(kind = r8), device, allocatable ::  rat_h2och4(:,:)
real(kind = r8), device, allocatable ::  rat_h2och4_1(:,:)
real(kind = r8), device, allocatable ::  rat_n2oco2(:,:)
real(kind = r8), device, allocatable ::  rat_n2oco2_1(:,:)
real(kind = r8), device, allocatable ::  rat_o3co2(:,:)
real(kind = r8), device, allocatable ::  rat_o3co2_1(:,:)

!************************************************************************
!-------------------------taumol_d use
real(kind = r8), device, allocatable ::  fracrefa01_d(:)
real(kind = r8), device, allocatable ::  fracrefb01_d(:)
real(kind = r8), device, allocatable ::  absa01_d(:,:)
real(kind = r8), device, allocatable ::  absb01_d(:,:)
real(kind = r8), device, allocatable ::  ka_mn201_d(:,:)
real(kind = r8), device, allocatable ::  kb_mn201_d(:,:)
real(kind = r8), device, allocatable ::  selfref01_d(:,:)
real(kind = r8), device, allocatable ::  forref01_d(:,:)

real(kind = r8), device, allocatable ::  fracrefa02_d(:)
real(kind = r8), device, allocatable ::  fracrefb02_d(:)
real(kind = r8), device, allocatable ::  absa02_d(:,:)
real(kind = r8), device, allocatable ::  absb02_d(:,:)
real(kind = r8), device, allocatable ::  selfref02_d(:,:)
real(kind = r8), device, allocatable ::  forref02_d(:,:)


real(kind = r8), device, allocatable ::  fracrefa03_d(:,:)
real(kind = r8), device, allocatable ::  fracrefb03_d(:,:)
real(kind = r8), device, allocatable ::  absa03_d(:,:)
real(kind = r8), device, allocatable ::  absb03_d(:,:)
real(kind = r8), device, allocatable ::  ka_mn2o03_d(:,:,:)
real(kind = r8), device, allocatable ::  kb_mn2o03_d(:,:,:)
real(kind = r8), device, allocatable ::  selfref03_d(:,:)
real(kind = r8), device, allocatable ::  forref03_d(:,:)

real(kind = r8), device, allocatable ::  fracrefa04_d(:,:)
real(kind = r8), device, allocatable ::  fracrefb04_d(:,:)
real(kind = r8), device, allocatable ::  absa04_d(:,:)
real(kind = r8), device, allocatable ::  absb04_d(:,:)
real(kind = r8), device, allocatable ::  selfref04_d(:,:)
real(kind = r8), device, allocatable ::  forref04_d(:,:)

real(kind = r8), device, allocatable ::  fracrefa05_d(:,:)
real(kind = r8), device, allocatable ::  fracrefb05_d(:,:)
real(kind = r8), device, allocatable ::  absa05_d(:,:)
real(kind = r8), device, allocatable ::  absb05_d(:,:)
real(kind = r8), device, allocatable ::  ka_mo305_d(:,:,:)
real(kind = r8), device, allocatable ::  selfref05_d(:,:)
real(kind = r8), device, allocatable ::  forref05_d(:,:)
real(kind = r8), device, allocatable ::  ccl405_d(:)

real(kind = r8), device, allocatable ::  fracrefa06_d(:)
real(kind = r8), device, allocatable ::  absa06_d(:,:)
real(kind = r8), device, allocatable ::  ka_mco206_d(:,:)
real(kind = r8), device, allocatable ::  selfref06_d(:,:)
real(kind = r8), device, allocatable ::  forref06_d(:,:)
real(kind = r8), device, allocatable ::  cfc11adj06_d(:)
real(kind = r8), device, allocatable ::  cfc1206_d(:)

real(kind = r8), device, allocatable ::  fracrefa07_d(:,:)
real(kind = r8), device, allocatable ::  fracrefb07_d(:)
real(kind = r8), device, allocatable ::  absa07_d(:,:)
real(kind = r8), device, allocatable ::  absb07_d(:,:)
real(kind = r8), device, allocatable ::  ka_mco207_d(:,:,:)
real(kind = r8), device, allocatable ::  kb_mco207_d(:,:)
real(kind = r8), device, allocatable ::  selfref07_d(:,:)
real(kind = r8), device, allocatable ::  forref07_d(:,:)

real(kind = r8), device, allocatable ::  fracrefa08_d(:) 
real(kind = r8), device, allocatable ::  fracrefb08_d(:)
real(kind = r8), device, allocatable ::  absa08_d(:,:)
real(kind = r8), device, allocatable ::  absb08_d(:,:)
real(kind = r8), device, allocatable ::  ka_mco208_d(:,:)
real(kind = r8), device, allocatable ::  ka_mn2o08_d(:,:)
real(kind = r8), device, allocatable ::  ka_mo308_d(:,:)
real(kind = r8), device, allocatable ::  kb_mco208_d(:,:)
real(kind = r8), device, allocatable ::  kb_mn2o08_d(:,:)
real(kind = r8), device, allocatable ::  selfref08_d(:,:)
real(kind = r8), device, allocatable ::  forref08_d(:,:)
real(kind = r8), device, allocatable ::  cfc1208_d(:)
real(kind = r8), device, allocatable ::  cfc22adj08_d(:)

real(kind = r8), device, allocatable ::  fracrefa09_d(:,:)
real(kind = r8), device, allocatable ::  fracrefb09_d(:)
real(kind = r8), device, allocatable ::  absa09_d(:,:)
real(kind = r8), device, allocatable ::  absb09_d(:,:)
real(kind = r8), device, allocatable ::  ka_mn2o09_d(:,:,:)
real(kind = r8), device, allocatable ::  kb_mn2o09_d(:,:)
real(kind = r8), device, allocatable ::  selfref09_d(:,:)
real(kind = r8), device, allocatable ::  forref09_d(:,:)


real(kind = r8), device, allocatable ::  fracrefa10_d(:)
real(kind = r8), device, allocatable ::  fracrefb10_d(:)
real(kind = r8), device, allocatable ::  absa10_d(:,:)
real(kind = r8), device, allocatable ::  absb10_d(:,:)
real(kind = r8), device, allocatable ::  selfref10_d(:,:)
real(kind = r8), device, allocatable ::  forref10_d(:,:)

real(kind = r8), device, allocatable ::  fracrefa11_d(:)
real(kind = r8), device, allocatable ::  fracrefb11_d(:)
real(kind = r8), device, allocatable ::  absa11_d(:,:)
real(kind = r8), device, allocatable ::  absb11_d(:,:)
real(kind = r8), device, allocatable ::  ka_mo211_d(:,:)
real(kind = r8), device, allocatable ::  kb_mo211_d(:,:)
real(kind = r8), device, allocatable ::  selfref11_d(:,:)
real(kind = r8), device, allocatable ::  forref11_d(:,:)

real(kind = r8), device, allocatable ::  fracrefa12_d(:,:)
real(kind = r8), device, allocatable ::  absa12_d(:,:)
real(kind = r8), device, allocatable ::  selfref12_d(:,:)
real(kind = r8), device, allocatable ::  forref12_d(:,:)


real(kind = r8), device, allocatable ::  fracrefa13_d(:,:)
real(kind = r8), device, allocatable ::  fracrefb13_d(:)
real(kind = r8), device, allocatable ::  absa13_d(:,:)
real(kind = r8), device, allocatable ::  ka_mco213_d(:,:,:)
real(kind = r8), device, allocatable ::  ka_mco13_d(:,:,:)
real(kind = r8), device, allocatable ::  kb_mo313_d(:,:)
real(kind = r8), device, allocatable ::  selfref13_d(:,:)
real(kind = r8), device, allocatable ::  forref13_d(:,:)


real(kind = r8), device, allocatable ::  fracrefa14_d(:)
real(kind = r8), device, allocatable ::  fracrefb14_d(:)
real(kind = r8), device, allocatable ::  absa14_d(:,:)
real(kind = r8), device, allocatable ::  absb14_d(:,:)
real(kind = r8), device, allocatable ::  selfref14_d(:,:)
real(kind = r8), device, allocatable ::  forref14_d(:,:)


real(kind = r8), device, allocatable ::  fracrefa15_d(:,:)
real(kind = r8), device, allocatable ::  absa15_d(:,:)
real(kind = r8), device, allocatable ::  ka_mn215_d(:,:,:)
real(kind = r8), device, allocatable ::  selfref15_d(:,:)
real(kind = r8), device, allocatable ::  forref15_d(:,:)


real(kind = r8), device, allocatable ::  fracrefa16_d(:,:)
real(kind = r8), device, allocatable ::  fracrefb16_d(:) 
real(kind = r8), device, allocatable ::  absa16_d(:,:)
real(kind = r8), device, allocatable ::  absb16_d(:,:)
real(kind = r8), device, allocatable ::  selfref16_d(:,:)
real(kind = r8), device, allocatable ::  forref16_d(:,:)

integer, device, allocatable ::  nspa_d(:)
integer, device, allocatable ::  nspb_d(:)

!-------------------------------taumol in

!-------------------------------taumol out
real(kind = r8), device, allocatable :: fracs(:,:,:)                                                   
real(kind = r8), device, allocatable :: taug(:,:,:)         
real(kind = r8), device, allocatable ::  taut(:,:,:)                                                    
!-----------------------------rtrnmc use
real(kind = r8), device, allocatable ::  delwave_d(:)
integer, device, allocatable ::  ngs_d(:)
real(kind = r8), device, allocatable ::  tau_tbl_d(:)
real(kind = r8), device, allocatable ::  exp_tbl_d(:)
real(kind = r8), device, allocatable ::  tfn_tbl_d(:)
!!!!integer, device, allocatable ::  ngb_d(:)
!-----------------------------rtrnmc in
 !real(kind = r8), device, allocatable ::  pz(:,:)
!real(kind = r8), device, allocatable ::  pwvcm(:) 
...
!real(kind = r8), device, allocatable ::  planklev(:,:,:)
!real(kind = r8), device, allocatable ::  plankbnd(:,:)
!real(kind=r8), device :: fracs(:,:,:) 
!real(kind = r8), device, allocatable ::  taut(:,:,:)
!integer, device :: ncbands(:)
!real(kind = r8), device, allocatable ::  cldfmc(:,:,:)
!real(kind = r8), device, allocatable ::  taucmc(:,:,:)


!------------------------------rtrnmc out 
real(kind = r8), device, allocatable ::  fnet(:,:)
real(kind = r8), device, allocatable ::  fnetc(:,:)
real(kind = r8), device, allocatable ::  totuflux(:,:)
real(kind = r8), device, allocatable ::  totdflux(:,:)
real(kind = r8), device, allocatable ::  htr(:,:)
real(kind = r8), device, allocatable ::  totuclfl(:,:)
real(kind = r8), device, allocatable ::  totdclfl(:,:)
real(kind = r8), device, allocatable ::  htrc(:,:)

!-----------------------------rtrnmc local
 real(kind = r8), device, allocatable ::  abscld(:,:,:)
 real(kind = r8), device, allocatable ::  odcld(:,:,:)
 real(kind = r8), device, allocatable ::  efclfrac(:,:,:)


 !real(kind = r8), device, allocatable ::  a0(:)
  !real(kind = r8), device, allocatable ::  a1(:)
  ! real(kind = r8), device, allocatable ::  a2(:)
!------------------------------out!!!!
real(kind = r8), device, allocatable ::  uflx_d(:,:)
real(kind = r8), device, allocatable ::  dflx_d(:,:)
real(kind = r8), device, allocatable ::  uflxc_d(:,:)
real(kind = r8), device, allocatable ::  dflxc_d(:,:)
real(kind = r8), device, allocatable ::  hr_d(:,:)
real(kind = r8), device, allocatable ::  hrc_d(:,:)
